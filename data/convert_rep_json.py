...
# Updated query: we now use the column 'abbreviation' from upac_states
query = """
...
for row in rows:
    ...
    # Determine the district key: if district_id is "00", designate it as "At-Large"
    district_key = "At-Large" if district_id == "00" else district_id

    # Create the composite key in the format "StateAbbreviation-District"
    composite_key = f"{abbreviation}-{district_key}"

    # Format the created_at field if it is a datetime object
